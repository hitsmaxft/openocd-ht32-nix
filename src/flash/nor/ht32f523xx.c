...
static int ht32f523xx_erase(struct flash_bank *bank, int first, int last)
{
    struct target *target = bank->target;
...
static int ht32f523xx_protect(struct flash_bank *bank, int set, int first, int last)
{
...
static int ht32f523xx_info(struct flash_bank *bank, char *buf, int buf_size)
{
    //ommand_print_sameline(cmd, "ht32f523xx");
    const char *info = "ht32f523xx flash";
